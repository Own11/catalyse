...
ALIASES = {"mit": "massachusetts institute of technology", "nyu": "new york university", "kaist": "kaist", "mbzuai": "mohamed bin zayed university of artificial intelligence"}


def identity_tokens(name: str) -> list[str]:
    expanded = ALIASES.get(name.strip().lower(), name)
    tokens = [x.lower() for x in expanded.replace("-", " ").split() if len(x) >= 4 and x.lower() not in {"university", "institute", "college", "of", "the"}]
...
def gemini_alias(name: str) -> str:
    ...
    fallback = ALIASES.get(name.strip().lower(), name)
...
def duckduckgo_images(name: str) -> list[dict]:
    try:
        ...
        with DDGS() as ddgs:
            results = []
            for query_category, category in (("campus", "campus"), ("dormitory", "dormitory"), ("library", "auditoriums"), ("auditorium", "auditoriums"), ("sports center", "sport"), ("students", "student_life")):
                results.extend({"image_url": x.get("thumbnail") or x.get("image"), "source_url": x.get("url"), "title": x.get("title", name) + " " + query_category, "university_match": True, "category": category} for x in ddgs.images(f"{name} official {query_category}", max_results=10) if x.get("image") and x.get("url"))
            return results
    except Exception:
        return []
...
def gemini_insights(profile: dict) -> dict:
    ...
    fallback = {"summary": f"{profile['name']} visual profile assembled from public sources.", "highlights": [k.replace('_', ' ').title() for k, v in profile["photos"].items() if v], "mode": "fallback"}
    key = os.getenv("GEMINI_API_KEY")
    if not key:
        return fallback
    prompt = f"Analyze {profile['name']} for a prospective student. Return JSON only with summary (2 sentences), highlights (3 short strings), recommendation (1 sentence). Photo counts: { {k: len(v) for k,v in profile['photos'].items()} }."
    ...
    try:
        ...
        with urlopen(request, timeout=8) as response:
            ...
            for page in pages:
                ...
                # Commons search is broad: require a distinctive university token too.
                name_tokens = identity_tokens(resolved_name)
                related = any(word in title for word in ("university", "campus", "college", "building", "library", "dorm", "hall", "sport", "student"))
                named = any(token in title for token in name_tokens)
                if url and related and named:
                    images.append({"image_url": url, "source_url": "https://commons.wikimedia.org/wiki/" + quote(page.get("title", "").replace(" ", "_")), "title": page.get("title", ""), "university_match": True})
    except Exception:
        pass
    if images:
